...
class TeamStakeholdersWeightingProposalsTable extends Component {
    ...
    updateStakeholder(row, fieldName, newValue) {
        ...
        if(fieldName.startsWith('proposal-'))
        {
            let proposalId = parseInt(fieldName.replace('proposal-', ''));
            let stakeholderId = row.id;
    ...
    render() {
        ...
        let mainColumnData = [
            ...
            },
            {
                ...
                editColumnClassName: null,
                invalidEditColumnClassName: null,
                tooltip: 'Each stakeholder has a vested interest in which proposal is ultimately selected.',
            }
            // {
        ...
        let extraColumnData = (this.props.proposalsLoading || _.isNil(this.props.proposals))  ? [] : this.props.proposals.map((d) => {

            let name = d.proposalText.substring(0, 10);

            if(name.length < d.proposalText.length)
                name = name.concat('...');

            if(this.props.symphysisModeEnabled)
                name = 'Weight: ' + d.proposalText;

            return {
                dataField: 'proposal-'+d.id.toString(),
                dataFormat: weightFormatter,
                name: name,
                hidden: false,
                hiddenOnInsert: false,
                //editable: {type: 'select', options: {values: [ -3, -2, -1, 0, 1, 2, 3]}},
                editable: this.props.updateStakeholderProposal ? {type: 'select', options: {values: [ -3, -2, -1, 0, 1, 2, 3]}} : false,
                tdStyle: null,
                editColumnClassName: null,
                invalidEditColumnClassName: null,
                tooltip: d.proposalText,
            }
...
TeamStakeholdersWeightingProposalsTable.propTypes = StakeholdersTablePropTypes;


export default TeamStakeholdersWeightingProposalsTable;
